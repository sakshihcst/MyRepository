...
public class RequestMPUDetailsMapper implements RowMapper<ItemDTO> {
...
//		itemDTO.setAssignedUser(rs.getString(ASSIGNED_USER.name()));
		...
		//storesys- 25790
		if(null != rs.getString(QTY.name()) && 
				null != rs.getString("detail_delivered_qty") && 
					(rs.getString(QTY.name()).equalsIgnoreCase(rs.getString("detail_delivered_qty")))) {
			itemDTO.setStockQuantity(null);
		}
